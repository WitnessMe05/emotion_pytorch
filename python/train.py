...
for sp in speak:
    ...
    class FeatureData(Dataset):
        ...
        def __init__(self, is_train_set=False, transforms=None):
            ...
            emo_sep = pd.concat([neu, ang, hap_m, sad]).reset_index(drop=True)
            self.len = emo_sep.shape[0]
            tmpdf = emo_sep.iloc[:,1:-3]
            self.filename = emo_sep.iloc[:,0]
            #print(tmpdf.head())
        ...
        def __getitem__(self, idx):
            return self.x_data[idx], self.y_data[idx], self.filename[idx]
    ...
    class CNN(torch.nn.Module):
        def __init__(self):
            ...
            self.linear1 = torch.nn.Linear(2352, 4, bias=True)
...
#     model_name = model_path + sp + "_" + str(epochs) + '_IS10_impBN'
...
# # RESULT OF 5 FOLD
# summary(model, input_size=(1, 1582))

...
def find_matching_label_file(wav_filename, labfiles, label_files_path):
    ...
    if len(wav_namepieces) == 3:
        #print(wav_namepieces)
        matchinglabfile = wav_namepieces[0] + '_' + wav_namepieces[1] + '.txt'
    else:
        #print(wav_namepieces)
        matchinglabfile = wav_namepieces[0] + "_" + wav_namepieces[1] + "_" + wav_namepieces[2] + ".txt"
...
def readlabtxt(lab_fullpath,n):
    ...
    labels = pd.read_table(lab_fullpath,header=0).iloc[0::n,:] # the summary is stored in every 8th row
